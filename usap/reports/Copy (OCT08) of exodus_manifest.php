...
echo com_siteheader('Exodus 2007 Manifest');
...
if(1)
{
    ...
    $setup['Augusta']['start'] = array('050000','140000','194500');

    //End times of flights for each wave
    $setup['Atlanta']['end'] = array('090059','125959','235959');
    $setup['Augusta']['end'] = array('135959','194459','194500');

    //Number of seats per bus to airport
        ...
        for($x=0;$x<$num;$x++)
        {
            ...
            if($airport=='Atlanta')
            {
                ...
                $query = "CREATE TEMPORARY TABLE temp1 (PRIMARY KEY(id)) SELECT e.id, e.dep_datetime
                          ...
                          e.exodus_status!='Returned' and e.dep_datetime between 20071220060000 AND 20071220235959
    ...
    foreach($_CONF['gate_numbers'] as $gate)
    {
        ...
        $query = "select m.id, m.Rank, m.Last_Name, m.First_Name, m.Middle_Initial as MI, right(m.ssn,4) as SSN,
                  ...
                  and e.dep_datetime BETWEEN 20071220000000 AND 20071220235959 and {$report['where']}
