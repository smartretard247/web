...
if(1)
{
    ...
    $setup['Atlanta']['depart'] = array('0001','0200','0400');
    $setup['Augusta']['depart'] = array('0300','0430','0630');

    //Start times of flights for each wave
    $setup['Atlanta']['start'] = array('050000','090100','130000');
    $setup['Augusta']['start'] = array('030000','080000','140000');

    //End times of flights for each wave
    $setup['Atlanta']['end'] = array('090000','125959','235959');
    $setup['Augusta']['end'] = array('075900','135900','235959');

    //Number of seats per bus for each airport
    $setup['Atlanta']['per_bus'] = 54;
    $setup['Augusta']['per_bus'] = 43;
    
    //Number of busses per wave
    $setup['Atlanta']['num_busses'] = 10;
    $setup['Augusta']['num_busses'] = 5;  //added 31Oct08

    //Start Number/Letter of busses
    //for each group
    $setup['Atlanta']['bus'][1] = 1;
    $setup['Augusta']['bus'][1] = 'A';
    $setup['Augusta']['bus'][2] = 'B';

    //Flag to reset bus numbers/letter
    //for each wave
    $setup['Atlanta']['reset_bus'] = TRUE;
    ...
    foreach($setup as $airport => $airport_data)
    {
        ...
        for($x=0;$x<$num;$x++)
        {
            ...
            if($airport=='Atlanta')
            {
                ...
                $query = "CREATE TEMPORARY TABLE temp1 (PRIMARY KEY(id)) SELECT e.id, e.dep_datetime
                          ...
                          e.exodus_status!='Returned' and e.dep_datetime between 20101218050000 AND 20101218235959
		...
		//$query = "select m.id, $order_column, '%%%' as Bus, CONCAT('#',m.battalion,'#') as Seat, m.Rank, m.Last_Name,
            ...
            else
            {
			  ...
			  //the following 3 lines were added 31Oct08 to test wave theory                
                          $soldiers_per_wave = $airport_data['per_bus'] * $airport_data['num_busses'];
